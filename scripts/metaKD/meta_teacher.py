...
from transformer.modeling import MetaTeacherForSequenceClassification
...
def do_eval(model, task_name, eval_dataloader,
    ...
    for batch_ in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            logits, domain_logits, *_ = model(input_ids, segment_ids, input_mask, domain_ids)
            domain_pred = torch.argmax(domain_logits, dim=-1)
            domain_preds.extend(domain_pred.tolist())
            domain_grts.extend(domain_ids.tolist())
    ...
    result['domain_acc'] = accuracy_score(domain_grts, domain_preds)
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained(
        args.pretrain_model_name_or_path, do_lower_case=args.do_lower_case)

    if not args.do_eval:
        if not args.aug_train:
            train_examples = processor.get_train_examples(args.data_dir, args.domain)
        else:
            train_examples = processor.get_aug_examples(args.data_dir, args.domain)
        ...
        cached_train_path = os.path.join(args.data_dir, "cached_train_features_{}{}{}{}.pt".format(
            args.domain, meta_str, "_with_weights" if args.use_sample_weights else "", portion_str))
    ...
    meta_teacher_model = MetaTeacherForSequenceClassification.from_pretrained(
        args.pretrain_model_name_or_path, num_labels=num_labels, num_domains=num_domains)
    ...
    if args.do_eval:
        ...
        for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                logits, domain_logits, *_ = meta_teacher_model(input_ids, segment_ids, input_mask, domain_ids)
                losses = ce_loss_fn(logits, label_ids)

                if args.use_domain_loss:
                    shuffled_domain_ids = domain_ids[torch.randperm(domain_ids.shape[0])]
                    domain_losses = ce_loss_fn(domain_logits, shuffled_domain_ids)
