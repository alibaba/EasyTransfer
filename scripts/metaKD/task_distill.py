...
class MnliProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, genre=None):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train.tsv")), "train", genre)

    def get_dev_examples(self, data_dir, genre=None):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev_matched.tsv")),
            "dev_matched", genre)

    def get_test_examples(self, data_dir, genre=None):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test_matched.tsv")),
            "test_matched", genre)

    def get_aug_examples(self, data_dir, genre=None):
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train_aug.tsv")), "aug", genre)
    ...
    def _create_examples(self, lines, set_type, genre=None):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            if genre and genre != "mix" and line[3] != genre:
                continue
            if cnt == 0:
...
class SentiProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, genre=None):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "train.tsv")), "train", genre)

    def get_dev_examples(self, data_dir, genre=None):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "dev.tsv")), "dev", genre)

    def get_test_examples(self, data_dir, genre=None):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "test.tsv")), "test", genre)
    ...
    def _create_examples(self, lines, set_type, genre=None):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = "%s-%s" % (set_type, line[0])
            text_a = review
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=None, label=sentiment))
        return examples
...
def do_eval(model, task_name, eval_dataloader,
    ...
    result["zero_rate"] = sum([t.item() == 0 for t in eval_labels]) / len(eval_labels)
...
def main():
    ...
    default_params = {
        ...
        "sent": {"num_train_epochs": 10, "max_seq_length": 128}
    }
    ...
    if not args.do_eval:
        if not args.aug_train:
            train_examples = processor.get_train_examples(args.data_dir, args.genre)
        else:
            train_examples = processor.get_aug_examples(args.data_dir, args.genre)
        ...
        if os.path.exists(os.path.join(args.data_dir, "cached_train_features_{}{}.pt".format(args.genre, portion_str))):
            train_features = torch.load(os.path.join(args.data_dir, "cached_train_features_{}{}.pt".format(args.genre, portion_str)))
        else:
            train_features = convert_examples_to_features(train_examples, label_list,
                                                          args.max_seq_length, tokenizer, output_mode)
            torch.save(train_features, os.path.join(args.data_dir, "cached_train_features_{}{}.pt".format(args.genre, portion_str)))
        train_data, _ = get_tensor_data(output_mode, train_features)
        train_sampler = RandomSampler(train_data)
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size)

    if args.do_eval:
        eval_examples = processor.get_test_examples(args.data_dir, args.genre)
    else:
        eval_examples = processor.get_dev_examples(args.data_dir, args.genre)
