...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            hidden_size: Size of the encoder modules and the pooler layer.
            num_hidden_layers: Number of hidden modules in the Transformer encoder.
            ...
            hidden_dropout_prob: The dropout probabilitiy for all fully connected
                modules in the embeddings, encoder, and pooler.
...
class MTLTinyBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, num_labels, num_domains, fit_size=768):
        super(MTLTinyBertForSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifiers = nn.ModuleList(
            [nn.Linear(config.hidden_size, num_labels) for _ in range(num_domains)])
        self.fit_dense = nn.Linear(config.hidden_size, fit_size)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                labels=None, is_student=False, domain_id=None):

        sequence_output, att_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                               output_all_encoded_layers=True, output_att=True)
        if domain_id is not None:
            logits = self.classifiers[domain_id](torch.relu(pooled_output))
            logits_list = logits
        else:
            logits_list = list()
            for classifer in self.classifiers:
                logits = classifer(torch.relu(pooled_output))
        ...
        if is_student:
            for s_id, sequence_layer in enumerate(sequence_output):
                tmp.append(self.fit_dense(sequence_layer))
            sequence_output = tmp
        return logits_list, att_output, sequence_output



class MetaTeacherForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, num_labels, num_domains):
        super(MetaTeacherForSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.domain_embedding = nn.Embedding(num_domains, config.hidden_size)
        self.corrupt_dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.domain_classifier = nn.Linear(config.hidden_size, num_domains)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, domain_ids=None):
        ...
        content_tensor = torch.mean(sequence_output[-1][:, 1:, :], dim=1)
        ...
        return logits, domain_logits, att_output, sequence_output

class MetaStudentForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, num_labels, num_domains, fit_size=768):
        super(MetaStudentForSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, domain_ids=None, is_student=False):
        ...
        domain_embedded = self.domain_embedding(domain_ids)
        content_tensor = torch.mean(sequence_output[-1][:, 1:, :], dim=1)
        domain_content_output = self.corrupt_dense(domain_embedded + content_tensor)
        if is_student:
            domain_content_output = self.fit_dense(domain_content_output)
        ...
        return logits, att_output, sequence_output, domain_content_output


class MetaTeacherWithProbes(MetaTeacherForSequenceClassification):
    def __init__(self, config, num_labels, num_domains, student_layer_num):
        super(MetaTeacherWithProbes, self).__init__(config, num_labels, num_domains)
        for param in self.parameters():
            param.requires_grad = False

        self.teacher_layer_num = config.num_hidden_layers
        self.student_layer_num = student_layer_num
        assert self.teacher_layer_num % self.student_layer_num == 0
        self.layers_per_block = int(self.teacher_layer_num / self.student_layer_num)
        self.probes = nn.ModuleList([nn.Linear(config.hidden_size, num_labels)
                                     for _ in range(self.student_layer_num)])

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, domain_ids=None):
        with torch.no_grad():
            sequence_output_list, att_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                                   output_all_encoded_layers=True,
                                                                   output_att=True)
        domain_embedded = self.domain_embedding(domain_ids)

        new_sequence_output_list = [sequence_output_list[i * self.layers_per_block]
                                    for i in range(self.student_layer_num)]
        probes_logits = list()
        for i, (sequence_output, probe) in enumerate(zip(new_sequence_output_list, self.probes)):
            if i == 0:
                hid_out = torch.tanh(torch.mean(sequence_output[:, 1:, :], dim=1))
            else:
                hid_out = self.bert.pooler([sequence_output])
            probes_logits.append(probe(hid_out))
        logits = self.classifier(torch.relu(pooled_output))

        content_tensor = torch.mean(sequence_output_list[-1][:, 1:, :])
        content_output = torch.tanh(self.corrupt_dense(domain_embedded + content_tensor))
        content_output = self.dropout(content_output)
        domain_logits = self.domain_classifier(content_output)
        return logits, probes_logits, domain_logits, att_output, pooled_output


class MetaStudentWithProbes(TinyBertForSequenceClassification):
    def __init__(self, config, num_labels):
        super(MetaStudentWithProbes, self).__init__(config, num_labels)
        self.probes = nn.ModuleList([nn.Linear(config.hidden_size, num_labels)
                                     for _ in range(config.num_hidden_layers)])

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, domain_embedded=None,
                labels=None, is_student=False):
        ...
        probes_logits = list()
        for i, (sequence_output, probe) in enumerate(zip(sequence_output_list[:-1], self.probes)):
            if i == 0:
                hid_out = torch.tanh(torch.mean(sequence_output[:, 1:, :], dim=1))
            else:
                hid_out = self.bert.pooler([sequence_output])
        ...
        tmp = []
        for s_id, sequence_layer in enumerate(sequence_output_list):
            tmp.append(self.fit_dense(sequence_layer))
        sequence_output_list = tmp
        return logits, probes_logits, att_output, sequence_output_list