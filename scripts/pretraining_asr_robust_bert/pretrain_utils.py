...
_app_flags.DEFINE_bool("multitaks_pretrain", default=False, help="multitaks_pretrain")
_app_flags.DEFINE_bool("encoder_decoder_pretrain", default=False, help="encoder_decoder_pretrain")
...
_app_flags.DEFINE_integer("save_steps", default=None, help='save_steps')
...
_app_flags.DEFINE_float("weight_decay_ratio", 0.01, "weight_decay_ratio")
_app_flags.DEFINE_integer("input_sequence_length", default=512, help='')
...
_app_flags.DEFINE_integer("attention_head_size", default=None, help='')
...
_app_flags.DEFINE_integer("top_k_number", default=30, help='')
    ...
    def __init__(self):
        ...
        config_json = {
            ...
            'train_config': {
                ...
                "save_steps": _APP_FLAGS.save_steps,
                "optimizer_config": {
                    "optimizer": "adam",
                    "weight_decay_ratio":_APP_FLAGS.weight_decay_ratio,
                    "warmup_ratio": 0.01,
        ...
        while i < len(document):
            ...
            if current_length >= max_num_tokens:
                if current_chunk:
                    ...
                    (tokens_with_mask, masked_lm_positions,
...
def convert_to_one_hot(input, indices, values):
    ret_indices = []
    ret_values = []
    for idx in input:
        ret_indices.extend(indices[idx])
        ret_values.extend(values[idx])
    return ret_indices, ret_values

def write_instance_to_file(instances, tokenizer, max_seq_length,
                           max_predictions_per_seq, output_file, ret_indices, ret_values, ret_indices_supervised, ret_values_supervised):
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        sparse_idxs, sparse_values = convert_to_one_hot(masked_lm_ids, ret_indices, ret_values)
        features["sparse_idxs"] = create_int_feature(sparse_idxs)
        features["sparse_values"] = create_float_feature(sparse_values)

        sparse_idxs_supervised, sparse_values_supervised = convert_to_one_hot(masked_lm_ids, ret_indices_supervised, ret_values_supervised)
        features["sparse_idxs_supervised"] = create_int_feature(sparse_idxs_supervised)
        features["sparse_values_supervised"] = create_float_feature(sparse_values_supervised)
