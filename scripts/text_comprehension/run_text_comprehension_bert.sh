...
#
# Easy Transfer AppZoo: Text Comprehension (BERT) Example
...
if [ ! -f ./train.csv ]; then
  wget http://atp-modelzoo-sh.oss-cn-shanghai.aliyuncs.com/tutorial/ez_text_comprehension/squad1.1/train.csv
fi

if [ ! -f ./dev.csv ]; then
  wget http://atp-modelzoo-sh.oss-cn-shanghai.aliyuncs.com/tutorial/ez_text_comprehension/squad1.1/dev.csv
fi
...
then
    ...
    easy_transfer_app \
      ...
      --sequenceLength=384 \
      --checkpointDir=./text_comprehension_models \
      --batchSize=32 \
      --numEpochs=1 \
      --optimizerType=adam \
      --learningRate=5e-5 \
...
then
    ...
    easy_transfer_app \
      ...
      --checkpointPath=./text_comprehension_models/ \
      --batchSize=32
else
    echo "invalid mode"
fi